...
from infer.predictor import Predictor,Predictor_normal,Predictor_noise,Predictor_normal_noise
from utils.visualize.plot_manager import PlotManager
from fit.fitter import Fitter,Fitter_normal,Fitter_noise,Fitter_normal_noise
import os
import time
import torch
import pathlib
...
config = dict(
    ...
    trainer=dict(
        ...
        samples_per_class=150,                                                                                      # "samples_per_class=3"表示每个类别（或者标签）下有多少个样本。
    ),
    predictor=dict(
        dataset='MNIST',
...
#     fitter_noise = Fitter_noise(**config['fitter'], plot_manager=plot_manager)
#     print('\nstart training\n')
#     trainer = Trainer(**config['trainer'], fitter=fitter)
#     classifier = trainer.train()   # 现在使用的随机森林分类器
#
#     print('\nstart testing\n')
#     predictor = Predictor_noise(**config['predictor'], classifier=classifier, fitter=fitter,fitter_noise=fitter_noise)
#     predictor.predict()
...
if __name__ == '__main__':
    ...
    fitter_noise = Fitter_normal_noise()
    ...
    print('\nstart testing\n')
    predictor = Predictor_normal_noise(**config['predictor'], classifier=classifier, fitter=fitter,fitter_noise=fitter_noise)
    predictor.predict()
    print(predictor.predicted)
    print(f'accuracy = {predictor.accuracy}')
